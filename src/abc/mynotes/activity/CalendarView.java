...
public class CalendarView extends ImageView {
    ...
    Drawable mDecWarning = null,  mDecDone = null;
    
    private List<Note> lstNoteRemind= null;
	...
	public CalendarView(Context context, AttributeSet attrs, int defStyle) {
		...
		mDecWarning = context.getResources().getDrawable(R.drawable.warning_calendar);
		mDecDone = context.getResources().getDrawable(R.drawable.check_calendar);
		initCalendarView();
	}
	...
	private void initCells() {
		...
		for(int week=0; week<mCells.length; week++) {
			for(int day=0; day<mCells[week].length; day++) {
				...
				List<Note> lstDayNotes= new ArrayList<Note>();
				for (Note _note : lstNoteRemind) {					
					Calendar dateremind = StringUtil.getCalendarDate( _note.getDateOfRemind());
					if(tmp[week][day].day== dateremind.get(Calendar.DATE) && mHelper.getYear() == dateremind.get(Calendar.YEAR) && mHelper.getMonth() == dateremind.get(Calendar.MONTH) ) {
						lstDayNotes.add(_note);
						
					}
				}
				mCells[week][day].setMarked(checkNoteFinish(lstDayNotes));
				mCells[week][day].setListNote(lstDayNotes);
	...
	protected void onDraw(Canvas canvas) {
		...
		for(Cell[] week : mCells) {
			for(Cell day : week) {
				day.draw(canvas);	
				Log.v("MARK", String.valueOf(day.getMarked()));
				if(day.getMarked() == 0){
					mDecWarning.setBounds(day.getBound());
					mDecWarning.draw(canvas);
				}
				else if(day.getMarked() == 1){
					mDecDone.setBounds(day.getBound());
					mDecDone.draw(canvas);
				}
			}
		}
	...
	private void getListNoteRemind(){
		DbAdapter db = new DbAdapter(this.getContext());
		try {
			db.open();
			lstNoteRemind = db.getAllRemindNotes();
			db.close();
		} catch (Exception e) {
	...
	private int checkNoteFinish(List< Note> lst){
		if(lst.size() == 0) return -1;
		for (Note note : lst) {
			if(note.getListTask() != null){
				for (Task task : note.getListTask()) {
